...
const getQuestionId = (index: number) => `faq-question-${index}`
const getAnswerId = (index: number) => `faq-answer-${index}`

// FAQPage JSON-LD Structured Data for SEO
const faqJsonLd = computed(() => {
  const items = t.value.faq.items
...
<template>
  ...
  >
    ...
    <div class="absolute -top-32 -left-32 w-96 h-96 bg-electric-cyan/10 rounded-full blur-3xl" aria-hidden="true" />
    <div class="absolute -bottom-32 -right-32 w-96 h-96 bg-valencia-purple/10 rounded-full blur-3xl" aria-hidden="true" />

    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      <!-- Section Title -->
      <div class="text-center mb-16">
        <h2 id="faq-heading" class="section-title text-gradient">{{ t.faq.title }}</h2>
        <div class="neon-divider" aria-hidden="true" />
      </div>

      <!-- FAQ Accordion -->
      <div class="space-y-4" role="list" :aria-label="t.faq.title">
        <div
        ...
        >
          ...
          <h3>
            ...
            >
              ...
              <svg
                class="flex-shrink-0 w-5 h-5 text-ocean-blue dark:text-electric-cyan transition-transform duration-300"
