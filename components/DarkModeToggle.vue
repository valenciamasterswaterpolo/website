...
<template>
  <button
    @click="toggleDark"
    class="p-2 rounded-full border border-soft-slate/30 hover:border-ocean-blue dark:hover:border-electric-cyan transition-all text-deep-navy dark:text-cream hover:bg-ocean-blue/10 dark:hover:bg-electric-cyan/10"
    :aria-label="isDark ? 'Switch to light mode' : 'Switch to dark mode'"
    :title="isDark ? 'Switch to light mode' : 'Switch to dark mode'"
  >
    <span v-if="isDark" v-html="sunIcon" />
    <span v-else v-html="moonIcon" />
  </button>
</template>
